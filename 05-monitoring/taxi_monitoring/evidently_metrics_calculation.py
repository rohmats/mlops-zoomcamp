...
def prep_db():
	with psycopg.connect("host=localhost port=5432 user=postgres password=example", autocommit=True) as conn:
		res = conn.execute("SELECT 1 FROM pg_database WHERE datname='taxi_db'")
		if len(res.fetchall()) == 0:
			conn.execute("create database taxi_db;")
		with psycopg.connect("host=localhost port=5432 dbname=taxi_db user=postgres password=example") as conn:
...
def batch_monitoring_backfill():
	...
	with psycopg.connect("host=localhost port=5432 dbname=taxi_db user=postgres password=example", autocommit=True) as conn:
		for i in range(0, 27):
